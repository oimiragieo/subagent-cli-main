...
program
  .name("subagent-cli")
  .description(
    "Enterprise AI Agent CLI Framework with maximum tool integration",
  )
  .version("1.0.0");
...
program
  .option(
    "-p, --platform <platform>",
    "Platform override (windows, macos, linux)",
    config.platform,
  )
  .option(
    "-s, --shell <shell>",
    "Shell override (bash, powershell, zsh)",
    config.defaultShell,
  )
  ...
  .action(async () => {
    ...
    try {
      ...
      info.availableShells.forEach((shell) => {
        console.log(
          chalk.cyan(`  ${shell.name}:`),
          shell.fullName,
          chalk.gray(`(${shell.version})`),
        );
      });

      console.log(chalk.bold("\n=== Default Shell ==="));
      console.log(
        chalk.green(`  ${info.defaultShell.name}:`),
        info.defaultShell.fullName,
      );
    } catch (error) {
    ...
    const answers = await inquirer.prompt([
      {
        ...
        choices: Object.keys(config.agents).filter(
    ...
    try {
      ...
      Object.entries(byCategory).forEach(([category, categoryTools]) => {
        ...
        categoryTools.forEach((tool) => {
          ...
          console.log(
            chalk.yellow(`  ${tool.name}`) + chalk.gray(` (${platforms})`),
          );
