...
export class CostTracker {
    ...
    async onMessage(message: AgentMessage, context?: TrackingContext): Promise<void> {
    ...
    generateReport(): CostReport {
        return {
            ...
            totalInputTokens: this.stepUsages.reduce((sum, s) => sum + s.usage.input_tokens, 0),
            totalOutputTokens: this.stepUsages.reduce((sum, s) => sum + s.usage.output_tokens, 0),
    ...
    private updateTodos(newTodos: TodoInput[]): void {
        ...
        this.todos = newTodos.map((todo, index) => {
            ...
            const todoObj: Todo = {
                ...
                startedAt: todo.status === "in_progress" && !existing?.startedAt
                    ? new Date()
                    : existing?.startedAt,
                completedAt: todo.status === "completed" && !existing?.completedAt
                    ? new Date()
...
export class BashTool {
    ...
    async execute(command: string, options?: BashExecuteOptions): Promise<BashResult> {
        // Validate command
    ...
    private async executeInContainer(
        ...
        if (result.stdout.length > this.config.maxOutputSize) {
            result.stdout = result.stdout.substring(0, this.config.maxOutputSize) +
    ...
    private timeout(ms: number): Promise<never> {
        return new Promise((_, reject) => {
            setTimeout(() => reject(new TimeoutError(`Command exceeded ${ms}ms timeout`)), ms);
        });
    }
}
```
...
export class CodeExecutionTool {
    ...
    async execute(code: string, config?: CodeExecutionConfig): Promise<CodeExecutionResult> {
...
export type EditorCommand = "view" | "str_replace" | "create" | "insert" | "undo_edit";
...
export class TextEditorTool {
    ...
    async strReplace(path: string, oldText: string, newText: string): Promise<ReplaceResult> {
        // Validate path
        this.validatePath(path);
    ...
    async insert(path: string, lineNumber: number, text: string): Promise<InsertResult> {
        // Validate path
        this.validatePath(path);
...
export const ENTERPRISE_AGENTS: AgentDefinitions = {
    "security-auditor": {
        description: "Performs comprehensive security audits including vulnerability assessment, code review, and compliance checks. ALWAYS use for security reviews and penetration testing analysis.",
...
[Provide prioritized action items]
    ...
    "test-engineer": {
        description: "Writes comprehensive test suites and executes tests. Use for test creation, test execution, coverage analysis, and quality assurance.",
...
Follow AAA pattern (Arrange, Act, Assert) and include descriptive test names.`,
    ...
    "documentation-writer": {
        description: "Creates clear, comprehensive technical documentation including README files, API documentation, architecture diagrams, and user guides.",
...
- Contributing guidelines`,
    ...
    "performance-optimizer": {
        description: "Analyzes and optimizes system performance including algorithm efficiency, database queries, caching, and resource utilization.",
...
- Implementation effort estimate`,
    ...
    "code-reviewer": {
        description: "Reviews code for quality, maintainability, and best practices. Use for code review, refactoring suggestions, and architectural guidance.",
...
- Priority levels (must-fix, should-fix, nice-to-have)`,
    ...
    "data-analyst": {
        description: "Analyzes data sets, performs statistical analysis, creates visualizations, and generates insights. Use for data exploration, reporting, and business intelligence.",
    ...
    getAgentStats(): AgentStats {
        ...
        for (const execution of this.executionLog) {
            ...
            if (execution.endTime && execution.startTime) {
                ...
                agentStat.avgDuration = agentStat.totalDuration / agentStat.totalExecutions;
            }
        }
